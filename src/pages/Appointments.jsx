...
import {
  ...
  ClipboardList, User, Stethoscope, Activity, PawPrint, CalendarCheck, Check, LayoutGrid
} from "lucide-react";
import {
...
export default function Appointments() {
  ...
  const [viewMode, setViewMode] = useState(() => localStorage.getItem("viewMode_appointments") || "list");
  const [currentPage, setCurrentPage] = useState(1);
  const ITEMS_PER_PAGE = 8;

  useEffect(() => {
    localStorage.setItem("viewMode_appointments", viewMode);
  }, [viewMode]);
  ...
  return (
    <div className="min-h-screen p-1.5 opacity-100 transition-opacity duration-500 text-left">
      ...
      <div className="bg-white rounded-2xl border border-gray-150 p-6 shadow-sm">
        ...
        {viewMode === "list" ? (
          <div className="overflow-x-auto rounded-xl border border-gray-50 animate-in fade-in duration-200">
            <Table>
              ...
              <TableBody>
                {filtered.length > 0 ? paginatedAppointments.map((item) => (
                  <TableRow key={item.id} className="hover:bg-emerald-50/30 transition-colors border-b-gray-50 group">
                    ...
                    <TableCell className="text-gray-650 text-sm">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 animate-in fade-in duration-200">
            {filtered.length > 0 ? paginatedAppointments.map((item) => (
              <div key={item.id} className="bg-white rounded-3xl border border-gray-150 p-6 shadow-sm hover:shadow-md transition-all duration-300 relative group flex flex-col justify-between">
                <div>
                  {/* Card Header: Patient & Status */}
                  <div className="flex items-center justify-between mb-4">
                    <div className="flex items-center gap-3">
                      <span className="w-10 h-10 rounded-full bg-emerald-50 text-emerald-600 flex items-center justify-center text-lg shadow-sm group-hover:scale-110 transition-transform">
                        {getPetEmoji(item.pets?.type)}
                      </span>
                      <div>
                        <h4 className="font-extrabold text-gray-805 text-sm">{item.pets?.name || "Hewan"}</h4>
                        <p className="text-[10px] text-gray-400 font-bold uppercase tracking-wider">{item.pets?.breed || "Campuran"}</p>
                      </div>
                    </div>
                    <span className={`px-2.5 py-1 rounded-full text-[9px] font-extrabold tracking-wider uppercase ${getStatusStyle(item.status)}`}>
                      {item.status}
                    </span>
                  </div>

                  {/* Details */}
                  <div className="space-y-3 my-4 pt-3 border-t border-gray-100">
                    <div className="flex items-center gap-2.5 text-xs text-gray-600">
                      <User className="w-4 h-4 text-gray-400" />
                      <span className="font-semibold text-gray-700">Pemilik: {item.owner?.full_name || "Pemilik"}</span>
                    </div>
                    <div className="flex items-center gap-2.5 text-xs text-gray-650">
                      <Stethoscope className="w-4 h-4 text-emerald-400" />
                      <span>Dokter: <span className="font-semibold">{item.doctor}</span></span>
                    </div>
                    <div className="flex items-center gap-2.5 text-xs text-gray-600">
                      <Calendar className="w-4 h-4 text-violet-400" />
                      <span>Jadwal: <span className="font-semibold text-gray-750">{item.date}</span> pada <span className="font-semibold text-gray-750">{item.time}</span></span>
                    </div>
                    <div className="flex items-center gap-2.5 text-xs text-gray-600">
                      <span className="bg-gray-150 text-gray-655 px-2.5 py-1 rounded-md text-xs font-semibold border border-gray-200">
                        {translateType(item.type)}
                      </span>
                    </div>
                    {item.notes && (
                      <div className="mt-3 p-3 bg-gray-50 rounded-2xl border border-gray-100 text-xs text-gray-500 italic">
                        Keluhan: "{item.notes}"
                      </div>
                    )}
                  </div>
                  ...
                  <div className="flex gap-2 mt-4 pt-3 border-t border-gray-100 justify-end">
                    {item.status === "Pending" && (
                      <>
                        <button
                          onClick={() => updateStatus(item.id, "Confirmed")}
                          className="px-3.5 py-2 bg-blue-500 hover:bg-blue-600 text-white rounded-xl text-xs font-bold cursor-pointer transition shadow-sm"
                        >
                          Konfirmasi
                        </button>
                        <button
                          onClick={() => updateStatus(item.id, "Cancelled")}
                          className="px-3.5 py-2 bg-rose-500 hover:bg-rose-600 text-white rounded-xl text-xs font-bold cursor-pointer transition shadow-sm"
                        >
                          Batal
                        </button>
                      </>
                    ...
                    {item.status === "Confirmed" && (
                      <>
                        <button
                          onClick={() => updateStatus(item.id, "Completed")}
                          className="px-3.5 py-2 bg-emerald-500 hover:bg-emerald-600 text-white rounded-xl text-xs font-bold cursor-pointer transition shadow-sm"
                        >
                          Selesai
                        </button>
                        <button
              ...
              <div className="bg-white border border-gray-150 rounded-3xl p-12 text-center text-gray-400 font-semibold w-full col-span-full">
                <div className="flex flex-col items-center justify-center gap-2">
                  <Search className="w-6 h-6 text-gray-300" />
                  <p>Tidak ada jadwal yang ditemukan.</p>
                </div>
